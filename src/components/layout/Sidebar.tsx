...
import TeacherRankBadge from "@/features/teacher/gamification/TeacherRankBadge";
import TeacherGamificationOverlay from "@/features/teacher/gamification/TeacherGamificationOverlay";
...
const TEACHER_NAV = [
  ...
  { path: "/teacher/profile",    icon: "👤", label: "Profil"     },
];
...
const PARENT_NAV = [
  ...
  { path: "/parent/profile",    icon: "👤", label: "Profil"       },
];
...
export function Sidebar() {
  ...
  return (
    <>
      {/* Gamification overlays — teacher only */}
      {isTeacher && <TeacherGamificationOverlay />}
      ...
      <aside className="sidebar">
        ...
        <nav style={{ flex: 1, display: "flex", flexDirection: "column", gap: 2 }}>
          {nav.map(item => (
            <NavLink
              key={item.path}
              to={item.path}
              end={item.path === "/" || item.path === "/teacher" || item.path === "/parent" || item.path === "/profile" || item.path === "/teacher/profile" || item.path === "/parent/profile"}
        ...
        {/* Teacher rank badge */}
        {isTeacher && <TeacherRankBadge />}

        {/* User footer */}
        <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "14px 10px", borderTop: "0.5px solid var(--border)", marginTop: isTeacher ? 0 : "auto" }}>
          <Avatar name={name} size="sm" onClick={() => navigate(isTeacher ? "/teacher/profile" : role === Role.Parent ? "/parent/profile" : "/profile")} />
