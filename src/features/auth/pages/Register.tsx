...
export default function Register() {
  ...
  const handleGoogleRegister = async () => {
    ...
    try {
      ...
      // Try backend firebase endpoint first (existing accounts)
      try {
        const res = await api.post<{ data: any }>('/api/auth/firebase', { token, role: selectedRole })
        const { setToken, setUser } = useAuthStore.getState()
        setToken(res.data.data.token, res.data.data.refreshToken ?? '')
        setUser(res.data.data.user ?? res.data.data)
        navigate('/', { replace: true })
        return
      } catch { /* new user — fall through to register */ }
      ...
      const res = await api.post<{ data: any }>('/api/auth/register', {
        ...
        email:     fbUser.email ?? '',
        password:  token.slice(0, 32),   // dummy password for Google users
        role:      selectedRole,
      ...
      setRegError('Google ilə qeydiyyat alınmadı. Yenidən cəhd edin.')
    } finally {
            ...
            {step === 1 ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  ...
                  <span style={{ fontSize: 11, color: 'var(--text-3)', fontWeight: 600 }}>və ya email ilə</span>
                  <div style={{ flex: 1, height: 1, background: 'var(--border)' }} />
                </div>
