...
import { apiUpdateProfile, apiGetSentPermissions, apiGetMyChildren } from '@/lib/api'
...
export default function Profile() {
  ...
  useEffect(() => {
    // publicProfiles-ı həmişə sinxronlaşdır ki, müəllim axtaranda tapsın
    const currentUid = auth.currentUser?.uid
    if (currentUid && userProfile) {
      dbSyncPublicProfile(
        {
          uniqueId: (userProfile as any).uniqueId,
  ...
  return (
    <div className="page-inner anim-fade-up">
      ...
      {/* ── Student section ─────────────────────────────────────────────────── */}
      {isStudent && (grade || group || city || school || foreignLang) && (
        <div className="card" style={{ padding: '18px 20px' }}>
      ...
      {isStudent && (
        <div className="dash-grid">
          <StatMini value={String(total ?? 0)} label="Sual" />
          <StatMini value={`${rate ?? 0}%`}    label="Ortalama" color="var(--success)" />
          <StatMini value={`${streak ?? 0}🔥`} label="Streak" />
        </div>
      )}
...
function PasswordModal({ onClose, email }: { onClose: () => void; email: string }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const msg = e?.code === 'auth/wrong-password' ? 'Cari şifrə yanlışdır'
                : e?.code === 'auth/too-many-requests' ? 'Çox cəhd. Bir az gözləyin'
